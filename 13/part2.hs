...
parse txt = let nextBlock = take 3 txt in go nextBlock : parse (drop 4 txt)
  where
    go :: [String] -> Machine
    go [a, b, p] =
      let button_a = map (read . filter isDigit) $ drop 2 $ words a :: [Integer]
          button_b = map (read . filter isDigit) $ drop 2 $ words b :: [Integer]
          prize = map ((+ 10000000000000) . read . filter isDigit) $ drop 1 $ words p :: [Integer]
...
calcPrice :: Maybe (Integer, Integer) -> Integer
...
solveMachine m =
  let -- matrix = a11 = A.X , a12 = B.X , a21 = A.Y , a22 = B.Y
      a = listArray ((1, 1), (2, 2)) [fst (buttonA m), fst (buttonB m), snd (buttonA m), snd (buttonB m)] :: Array (Int, Int) Integer
      det_a = determinant a
      a1 = listArray (bounds a) [fst $ prize m, a ! (1, 2), snd $ prize m, a ! (2, 2)] :: Array (Int, Int) Integer
      a2 = listArray (bounds a) [a ! (1, 1), fst $ prize m, a ! (2, 1), snd $ prize m] :: Array (Int, Int) Integer
